...
\begin{rSection}{项目与技术栈}
    ...
    \begin{itemize}
        ...
        \item 使用的技术栈包括 Julia、HTML/CSS/JavaScript、Swift等。
    \end{itemize}
    主导开发了\href{https://github.com/sujunyan/ParExMPC}{\textcolor{blue}{ParExMPC}}:
    \begin{itemize}
        \item ParExMPC 是一款模型预测控制（MPC）设计工具箱。给定一个非线性系统模型和一个优化目标，用户可通过MATLAB界面生成一个 轻量MPC 控制器。相关成果已发表论文。
        \item 可生成C代码。生成的代码可部署在最低内存2kb嵌入式设备上。
        \item 使用技术栈包括MATLAB、C。
        % \item 提供基于 MATLAB 的用户界面和定制的 C 代码求解器。
    \end{itemize}

    主导参加\href{https://www.robomaster.com/zh-CN}{\textcolor{blue}{RoboMaster 2018 机甲大师赛}}（电控部分）:
    \begin{itemize}
        \item 本人在团队中全面负责电控系统，涵盖嵌入式软件、硬件及整机电子架构。基于官方推荐的 STM32F4xx 开发版，引入 FreeRTOS 多线程框架，自底向上实现驱动层、控制层与指令级功能。
